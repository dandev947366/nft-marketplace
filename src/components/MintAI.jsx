...
const CreateNFT = () => {
  ...
  const onClose = () => {
    resetForm();
    setGlobalState("aimintModal", "scale-0");
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-screen h-screen flex items-center justify-center bg-black bg-opacity-0 transform transition-transform duration-500 ${aimintModal}`}
    >
      <div className="bg-[#303949] relative flex flex-col w-full max-w-3xl mx-auto my-8 rounded-md shadow dark:shadow-gray-100 shadow-primary-500 text-white w-11/12 sm:w-2/5 h-7/12 p-6">
        <form className="flex flex-col items-center" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="flex bg-indigo-600 justify-center items-center w-[200px] text-white text-md py-2 px-5 mb-2 rounded-full drop-shadow-xl border border-transparent hover:text-white" onClick={createImage}
          >
            Create NFT
          </button>
          <button
            type="submit"
            className="flex bg-indigo-600 justify-center items-center w-[200px] text-white text-md py-2 px-5 rounded-full drop-shadow-xl border border-transparent hover:text-white mt-2"
          >
            Mint
          </button>
        </form>
